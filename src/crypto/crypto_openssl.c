...
int md4_vector(size_t num_elem, const uint8_t *addr[], const size_t *len,
               uint8_t *mac) {
...
int rc4_skip(const uint8_t *key, size_t keylen, size_t skip, uint8_t *data,
             size_t data_len) {
...
int sha256_vector(size_t num_elem, const uint8_t *addr[], const size_t *len,
                  uint8_t *mac) {
  return openssl_digest_vector(EVP_sha256(), 0, num_elem, addr, len, mac);
}
...
void aes_encrypt_deinit(void *ctx) { free(ctx); }
...
void aes_decrypt_deinit(void *ctx) { free(ctx); }

int crypto_mod_exp(const uint8_t *base, size_t base_len, const uint8_t *power,
...
int crypto_cipher_encrypt(struct crypto_cipher *ctx, const uint8_t *plain,
...
int crypto_cipher_decrypt(struct crypto_cipher *ctx, const uint8_t *crypt,
...
//
//
// struct wpabuf * dh5_derive_shared(void *ctx, const struct wpabuf
// *peer_public,
