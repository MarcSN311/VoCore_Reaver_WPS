...
int wpabuf_resize(struct wpabuf **_buf, size_t add_len) {
  ...
  if (buf->used + add_len > buf->size) {
    unsigned char *nbuf;
    if (buf->ext_data) {
      nbuf = realloc(buf->ext_data, buf->used + add_len);
      if (nbuf == NULL)
        return -1;
      memset(nbuf + buf->used, 0, add_len);
      buf->ext_data = nbuf;
    } else {
      nbuf = realloc(buf, sizeof(struct wpabuf) + buf->used + add_len);
      if (nbuf == NULL)
        return -1;
      buf = (struct wpabuf *)nbuf;
      memset(nbuf + sizeof(struct wpabuf) + buf->used, 0, add_len);
      *_buf = buf;
    }
...
struct wpabuf *wpabuf_alloc(size_t len) {
  struct wpabuf *buf = zalloc(sizeof(struct wpabuf) + len);
  if (buf == NULL)
    return NULL;
...
struct wpabuf *wpabuf_alloc_ext_data(uint8_t *data, size_t len) {
  struct wpabuf *buf = zalloc(sizeof(struct wpabuf));
  if (buf == NULL)
    return NULL;
